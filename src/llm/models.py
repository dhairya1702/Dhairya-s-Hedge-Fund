...
AVAILABLE_MODELS = [
    ...
    LLMModel(
        display_name="[together] mixtral",
        model_name="mistralai/Mixtral-8x7B-Instruct-v0.1",
        provider=ModelProvider.TOGETHER
    ),
    LLMModel(
        display_name="[together] llama-3-8b",
        model_name="meta-llama/Llama-3-8b-chat-hf",
        provider=ModelProvider.TOGETHER
    ),
    LLMModel(
        display_name="[ollama] Dhairya's llama3",
        model_name="llama3",
...
def get_model(model_name: str, model_provider: ModelProvider) -> ChatOpenAI | ChatGroq | None:
    if model_provider == ModelProvider.GROQ:
        ...
        from src.llm.chat_together import ChatTogether
        api_key = os.getenv("TOGETHER_API_KEY")
        if not api_key:
            print(f"API Key Error: Please make sure TOGETHER_API_KEY is set in your .env file.")
            raise ValueError("Together API key not found.")
        return ChatTogether(model=model_name, api_key=api_key)

    elif model_provider == ModelProvider.OLLAMA:
        from src.llm.chat_ollama import ChatOllama
