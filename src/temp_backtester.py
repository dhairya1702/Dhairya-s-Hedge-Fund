...
from agents.temp_sentiment import temp_sentiment_agent
...
init(autoreset=True)
sentiment_history = defaultdict(list)  # {ticker: list of (date, score)}
ROLLING_WINDOW = 5  # (Change 5 to any window you want)
    ...
    def run_backtest(self):
        ...
        for current_date in dates:
            ...
            if args.use_sentiment:
                progress.update_status("system", "all", f"Running Sentiment Analysis for {current_date_str}...")
                sentiment_result = sentiment_agent(state, date=current_date)
                state["data"] = sentiment_result["data"]
                print("we are in backtesting file bro")
                print("sentiment_result['data']['sentiment_scores']:", sentiment_result["data"].get("sentiment_scores"))
                print("state['data']['sentiment_scores']:", state["data"].get("sentiment_scores"))
                print("tickers:", self.tickers)  # or tickers, whichever you use!

                # STEP 2: Save each ticker's score
                for ticker, sentiment in state["data"].get("sentiment_scores", {}).items():
                    sentiment_history[ticker].append((current_date_str, sentiment["score"]))
                    # 👇👇 Add this DEBUG print right here 👇👇
                if "sentiment_scores" in state["data"]:
                    for ticker in self.tickers:
                        print(
                                f"[DEBUG] Agent state before {self.agent.__name__} for {ticker} on {current_date_str}:",
                                state["data"]["sentiment_scores"].get(ticker))
                else:
                    print(f"[DEBUG] No sentiment_scores in state for {current_date_str}")

                # STEP 3: Compute and store rolling average for each ticker
                for ticker in state["data"]["sentiment_scores"]:
                    history = [score for date, score in sentiment_history[ticker] if date <= current_date_str]
                    rolling_scores = history[-ROLLING_WINDOW:]
                    if rolling_scores:
                        rolling_avg = sum(rolling_scores) / len(rolling_scores)
                    else:
                        rolling_avg = None

                    state["data"]["sentiment_scores"][ticker]["rolling_avg"] = round(rolling_avg,
                                                                                     4) if rolling_avg is not None else None
            ...
            # ---------------------------------------------------------------
            #print("[DEBUG] BEFORE agent call, sentiment_scores =", state["data"].get("sentiment_scores"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use-sentiment",
        action="store_true",
        help="Enable sentiment analysis during backtesting",
    )
    ...
    state = {
        "data": {
            "tickers": args.tickers.split(","),  # make sure tickers are comma-separated
            "start_date": args.start_date,
            "end_date": args.end_date,
            "analyst_signals": {},  # ← REQUIRED
        },
        "metadata": {
            "model_name": "llama3-70b-8192",  # or whatever model you're using
            "model_provider": "groq",
            "show_reasoning": True,
        }
    }
    ...
    if not choices:
        ...
        print("hello boi,",selected_analysts)
        print(
            f"\nSelected analysts: "
