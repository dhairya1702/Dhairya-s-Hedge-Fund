...
def warren_buffett_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        sentiment_data = state["data"].get("sentiment_scores", {}).get(ticker)
        print(f"[debugin BUffet Sar] Sentiment in state for {ticker}: {sentiment_data}")
        if sentiment_data:
            analysis_data[ticker]["sentiment_summary"] = sentiment_data["summary"]
            analysis_data[ticker]["sentiment_score"] = sentiment_data["score"]
            # Prefer rolling average; fallback to raw score if not available
            #state["data"]["sentiment_scores"][ticker]["score"]
            print(f"[still in buffet] Sentiment score for {ticker}: {sentiment_data.get('score')}")
    ...
    template = ChatPromptTemplate.from_messages(
        [
            ...
            (
              ...
              If 'sentiment_summary' or 'sentiment_score' is included, take it into account when evaluating the company's prospects. 
              For example, if sentiment is overly negative despite strong fundamentals, note potential opportunity. 
              If sentiment is euphorically positive while fundamentals are weak, consider the risk of overvaluation.
              Explicitly explain in your reasoning how it affects your investment signal.
